...
import {
  ...
  getDeliveredArrangementReminderIds,
  manualStorageKey,
  preferenceStorageKey,
  readJsonArrayValue,
  setDeliveredArrangementReminderIds,
} from "@/data/arrangementDemoTools";
import { buildArrangements } from "@/data/arrangements";
import type { StoredArrangementPreference, StoredManualArrangement } from "@/data/arrangements";
import { useCandidateProfile } from "@/data/candidateProfile";
import {
  arrangementReminderMessagePrefix,
  createArrangementReminderMessage,
...
const arrangementReminderSettingsStorageKey = "arkme-demo.arrangements.reminderSettings";
...
type ArrangementReminderSettings = {
  sms: boolean;
  vibration: boolean;
  phone: boolean;
};

function getInitialArrangementReminderSettings(): ArrangementReminderSettings {
  if (typeof window === "undefined") {
    return { sms: true, vibration: true, phone: false };
  }

  try {
    const storedValue = window.localStorage.getItem(arrangementReminderSettingsStorageKey);
    if (!storedValue) return { sms: true, vibration: true, phone: false };
    const parsed = JSON.parse(storedValue) as Partial<ArrangementReminderSettings>;
    return {
...
function getArrangementReminderMinutes(item: { hour?: number; minute?: number }) {
  const hour = typeof item.hour === "number" ? item.hour : 6;
...
export default function Home({ currentPage, onNavigate }: HomeProps) {
  ...
  const [settingsView, setSettingsView] = React.useState<
    null | "settings" | "appearance" | "arrangements" | "about"
  >(null);
  ...
  const [arrangementNow, setArrangementNow] = React.useState(() => new Date());
  ...
  React.useEffect(() => {
    ...
    const refreshArrangementNow = () => setArrangementNow(new Date());
    const intervalId = window.setInterval(refreshArrangementNow, 30 * 1000);
    window.addEventListener("storage", refreshArrangementNow);
    window.addEventListener(arrangementDemoStateChangedEvent, refreshArrangementNow);
    return () => {
      window.clearInterval(intervalId);
      window.removeEventListener("storage", refreshArrangementNow);
      window.removeEventListener(arrangementDemoStateChangedEvent, refreshArrangementNow);
    };
  }, []);
  ...
  React.useEffect(() => {
    const manualItems = readJsonArrayValue<StoredManualArrangement>(manualStorageKey);
    const preferences = readJsonArrayValue<StoredArrangementPreference>(preferenceStorageKey);
    const deletedGeneratedIds = readJsonArrayValue<string>(deletedGeneratedStorageKey);
    const acceptedLowConfidenceIds = readJsonArrayValue<string>(acceptedLowConfidenceStorageKey);
    const activeArrangements = buildArrangements({
      manualItems,
      preferences,
      selfRecords,
      testMessages,
      ...
      (item) =>
        !deletedGeneratedIds.includes(item.id) &&
        (!item.generated || item.confidence === "high" || acceptedLowConfidenceIds.includes(item.id)) &&
        item.status !== "completed" &&
        item.reminder === "on" &&
        item.dateKey === formatDateKey(arrangementNow) &&
        arrangementNow.getHours() * 60 + arrangementNow.getMinutes() >= getArrangementReminderMinutes(item)
    );

    if (activeArrangements.length === 0) return;

    const existingReminderIds = new Set(
      testMessages
        .filter((message) => message.id.startsWith(arrangementReminderMessagePrefix))
        .map((message) => message.id)
    );
    const deliveredReminderIds = getDeliveredArrangementReminderIds();
    const deliveredReminderSet = new Set(deliveredReminderIds);
    const dueArrangements = activeArrangements.filter((item) => !deliveredReminderSet.has(item.id));

    if (dueArrangements.length === 0) return;

    const timestamp = arrangementNow.getTime();
    const reminderMessages = dueArrangements
      .map((item, index) => {
        const message = createArrangementReminderMessage(
          item.id,
          `事务提醒：${item.title}`,
          timestamp + index
        );
        return existingReminderIds.has(message.id) ? null : message;
      })
      .filter((message): message is TestMessage => Boolean(message));

    if (reminderMessages.length === 0) {
      setDeliveredArrangementReminderIds([...deliveredReminderIds, ...dueArrangements.map((item) => item.id)]);
      return;
    }
    ...
    setTestMessages((prev) => {
      ...
      const nextMessages = [
        ...prev,
        ...reminderMessages.filter((message) => !prevIds.has(message.id)),
      ];
      persistTestMessages(nextMessages);
      return nextMessages;
    });
    setDeliveredArrangementReminderIds([...deliveredReminderIds, ...dueArrangements.map((item) => item.id)]);
  }, [arrangementNow, selfRecords, testGroups, testIdentities, testMessages]);
  ...
  const renderMainContent = () => {
    ...
    if (settingsView === "arrangements") {
      return <ArrangementSettingsScreen onBack={() => setSettingsView("settings")} />;
    }
    ...
    if (settingsView === "settings") {
      return (
        <SettingsScreen
          ...
          onOpenArrangements={() => setSettingsView("arrangements")}
        />
      );
    }
...
function ArrangementSettingsScreen({ onBack }: { onBack: () => void }) {
  const [arrangementReminderSettings, setArrangementReminderSettings] = React.useState(
    getInitialArrangementReminderSettings
  );

  React.useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(
      arrangementReminderSettingsStorageKey,
      JSON.stringify(arrangementReminderSettings)
    );
  }, [arrangementReminderSettings]);

  const toggleArrangementReminder = (key: keyof ArrangementReminderSettings) => {
    setArrangementReminderSettings((current) => ({
      ...current,
      [key]: !current[key],
    }));
  };
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
      <MobilePageHeader title="事务设置" onBack={onBack} />

      <div className="min-h-0 flex-1 overflow-y-auto px-2.5 py-3">
        <section className="overflow-hidden rounded-[12px] bg-surface">
          <div className="border-b border-border-light px-3 py-3">
            <h2 className="text-[15px] font-semibold leading-5 text-text">事务提醒</h2>
            <p className="mt-1 text-xs leading-4 text-text-tertiary">
              设置安排事务使用系统好友消息、震动或电话提醒的边界。
            </p>
          </div>
          <SettingsToggleItem
            title="安排好友消息提醒"
            description="开启后，绿色事务和每日汇总会由“安排”发消息触达"
            checked={arrangementReminderSettings.sms}
            onChange={() => toggleArrangementReminder("sms")}
          />
          <SettingsToggleItem
            title="事务提醒震动"
            description="开启后，橙色事务可使用震动；未定时事务默认不震动"
            checked={arrangementReminderSettings.vibration}
            onChange={() => toggleArrangementReminder("vibration")}
          />
          <SettingsToggleItem
            title="事务提醒电话"
            description="仅用户主动设为红色事务时使用"
            checked={arrangementReminderSettings.phone}
            onChange={() => toggleArrangementReminder("phone")}
          />
        </section>

        <section className="mt-3 rounded-[12px] bg-surface px-3 py-3">
          <h2 className="text-[15px] font-semibold leading-5 text-text">默认提醒策略</h2>
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            没有具体时分的事务，会在 06:00 由“安排”好友发消息提醒“查看您今日所需事务”，不使用震动或电话。
          </p>
        </section>
      </div>
    </div>
...
function SettingsToggleItem({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "ml-3 flex h-7 w-12 shrink-0 items-center rounded-full p-1 transition",
